...
func (e *emitter) augmentNode(node *typeDocNode, parent *typeDocNode) {
	...
	node.CommentText = cleanComment(node.Comment.Text)
	node.CommentShortText = cleanComment(node.Comment.ShortText)
...
// cleanComment just applies some transformations to make the comments format nicer in our docs.
func cleanComment(c string) string {
	var result string
	lines := strings.Split(c, "\n")
	for _, line := range lines {
		if strings.Index(line, "## ") == 0 {
			result += fmt.Sprintf("<h3 class='pdoc-apis'>%s</h3>\n", line[3:])
		} else if strings.Index(line, "### ") == 0 {
			result += fmt.Sprintf("<h4 class='pdoc-apis'>%s</h4>\n", line[4:])
		} else {
...
type typeDocNode struct {
	...
	AnchorName string
	// CommentText is the Comment.Text, with some transformations applied for formatting purposes.
	CommentText string
	// CommentShortText is the Comment.ShortText, with some transformations applied for formatting purposes.
