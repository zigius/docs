...
<dl class="resources-properties">
    ...
    <dd>Do not validate the certificate when monitor use HTTPS. Only valid if `type` is "http" or "https".
</dd>
    ...
    <dd>A case-insensitive sub-string to look for in the response body. If this string is not found, the origin will be marked as unhealthy. Only valid if `type` is "http" or "https". Default: "".
</dd>
    ...
    <dd>The expected HTTP response code or code range of the health check. Eg `2xx`. Only valid and required if `type` is "http" or "https".
</dd>
    ...
    <dd>Follow redirects if returned by the origin. Only valid if `type` is "http" or "https".
</dd>
    ...
    <dd>The interval between each health check. Shorter intervals may improve failover time, but will increase load on the origins as we check from multiple locations. Default: 60.
</dd>
    ...
    <dd>The method to use for the health check. Valid values are any valid HTTP verb if `type` is "http" or "https", or `connection_established` if `type` is "tcp". Default: "GET" if `type` is "http" or "https", or "connection_established" if `type` is "tcp" .
</dd>
    ...
    <dd>The endpoint path to health check against. Default: "/". Only valid if `type` is "http" or "https".
</dd>
    ...
    <dd>The number of retries to attempt in case of a timeout before marking the origin as unhealthy. Retries are attempted immediately. Default: 2.
</dd>
    ...
    <dd>The timeout (in seconds) before marking the health check as failed. Default: 5.
</dd>
    ...
    <dd>The protocol to use for the healthcheck. Currently supported protocols are 'HTTP', 'HTTPS' and 'TCP'. Default: "http".
</dd>
...
<dl class="resources-properties">
    ...
    <dd>The RFC3339 timestamp of when the load balancer monitor was created.
</dd>
    ...
    <dd>The RFC3339 timestamp of when the load balancer monitor was last modified.
</dd>
...
<dl class="resources-properties">
    ...
    <dd>A list of string values for the header.
</dd>
