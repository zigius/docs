...
<dl class="resources-properties">
    ...
    <dd>The listener frontend port which is used to launch the new forwarding rule. Valid range: [1-65535].
</dd>
    ...
    <dd>The Load Balancer ID which is used to launch the new forwarding rule.
</dd>
    ...
    <dd>ID of a virtual server group that will be forwarded.
</dd>
    ...
    <dd>Domain name of the forwarding rule. It can contain letters a-z, numbers 0-9, hyphens (-), and periods (.),
...
- Wildcard domain name: *.test.com. wildcard (*) must be the first character in the format of (*.)
    ...
    <dd>Whether to enable health check. Valid values are`on` and `off`. TCP and UDP listener's HealthCheck is always on, so it will be ignore when launching TCP or UDP listener. This parameter is required  and takes effect only when ListenerSync is set to off.
</dd>
    ...
    <dd>Indicates whether a forwarding rule inherits the settings of a health check , session persistence, and scheduling algorithm from a listener. Default to on.
</dd>
    ...
    <dd>Name of the forwarding rule. Our plugin provides a default name: "tf-slb-rule".
</dd>
    ...
    <dd>Scheduling algorithm, Valid values are `wrr`, `rr` and `wlc`.  Default to "wrr". This parameter is required  and takes effect only when ListenerSync is set to off.
</dd>
    ...
    <dd>Whether to enable session persistence, Valid values are `on` and `off`. Default to `off`. This parameter is required  and takes effect only when ListenerSync is set to off.                                                                                                                                                                                                                                                 
</dd>
    ...
    <dd>Domain of the forwarding rule. It must be 2-80 characters in length. Only letters a-z, numbers 0-9,
