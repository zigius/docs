...
<dl class="resources-properties">
    ...
    <dd>The SNAT ip address, the ip must along bandwidth package public ip which `alicloud.vpc.NatGateway` argument `bandwidth_packages`.
</dd>
    ...
    <dd>The value can get from `alicloud.vpc.NatGateway` Attributes "snat_table_ids".
</dd>
    ...
    <dd>The private network segment of Ecs. This parameter and the `source_vswitch_id` parameter are mutually exclusive and cannot appear at the same time.
</dd>
...
<dl class="resources-properties">
    ...
    <dd>The id of the snat entry on the server.
</dd>
