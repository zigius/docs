...
<dl class="resources-properties">
    ...
    <dd>Environment variables available to the application.
</dd>
    ...
    <dd>An ordered list of URL-matching patterns that should be applied to incoming requests. The first matching URL handles the
request and other request handlers are not attempted.
    ...
    <dd>Instance class that is used to run this version. Valid values are AutomaticScaling F1, F2, F4, F4_1G (Only
AutomaticScaling is supported at the moment)
    ...
    <dd>Configuration for third-party Python runtime libraries that are required by the application.
</dd>
...
https://cloud.google.com/appengine/docs/standard//config/appref
    ...
    <dd>Whether multiple requests can be dispatched to this version at once.
</dd>
