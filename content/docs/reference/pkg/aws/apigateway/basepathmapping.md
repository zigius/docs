...
<dl class="resources-properties">
    ...
    <dd>The already-registered domain name to connect the API to.
</dd>
    ...
    <dd>The id of the API to connect.
</dd>
    ...
    <dd>Path segment that must be prepended to the path when accessing the API via this mapping. If omitted, the API is exposed at the root of the given domain.
</dd>
    ...
    <dd>The name of a specific deployment stage to expose at the given path. If omitted, callers may select any stage by including its name as a path element after the base path.
</dd>
