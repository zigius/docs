...
<dl class="resources-properties">
    ...
    <dd>The ARN of the destination to deliver matching log events to. Kinesis stream or Lambda function ARN.
</dd>
    ...
    <dd>A valid CloudWatch Logs filter pattern for subscribing to a filtered stream of log events.
</dd>
    ...
    <dd>The name of the log group to associate the subscription filter with
</dd>
    ...
    <dd>The method used to distribute log data to the destination. By default log data is grouped by log stream, but the grouping can be set to random for a more even distribution. This property is only applicable when the destination is an Amazon Kinesis stream. Valid values are "Random" and "ByLogStream".
</dd>
