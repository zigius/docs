...
<dl class="resources-properties">
    ...
    <dd>The start port (or ICMP type number if protocol is "icmp" or "icmpv6").
</dd>
    ...
    <dd>The protocol. If not icmp, icmpv6, tcp, udp, or all use the [protocol number](https://www.iana.org/assignments/protocol-numbers/protocol-numbers.xhtml)
</dd>
    ...
    <dd>The end port (or ICMP code if protocol is "icmp").
</dd>
    ...
    <dd>The type of rule being created. Valid options are `ingress` (inbound)
or `egress` (outbound).
</dd>
    ...
    <dd>List of CIDR blocks. Cannot be specified with `source_security_group_id`.
</dd>
    ...
    <dd>List of prefix list IDs (for allowing access to VPC endpoints).
Only valid with `egress`.
    ...
    <dd>If true, the security group itself will be added as
a source to this ingress rule. Cannot be specified with `source_security_group_id`.
    ...
    <dd>The security group id to allow access to/from,
